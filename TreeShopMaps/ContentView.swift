...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .actionSheet(isPresented: $showWorkAreaActionSheet) {
            ActionSheet(
                title: Text(selectedWorkArea?.name ?? "Work Area"),
                message: Text("Area: \(String(format: "%.2f", selectedWorkArea?.area ?? 0)) acres"),
                buttons: [
                    .destructive(Text("Delete")) {
                        if let workArea = selectedWorkArea {
                            deleteWorkArea(workArea)
                        }
                    },
                    .default(Text("View Details")) {
                        // Keep work area selected for details view
                    },
                    .cancel() {
